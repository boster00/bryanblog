...
const CANONICAL_PROMPT_PREFIX = [
  'Editorial diagram for a retirement-planning content site called "The Mysterious Author".',
  "Style: clean editorial vector illustration / schematic / infographic.",
  "Palette: deep slate teal-navy #1C4259, muted oxblood-terracotta #8C3A2E,",
  "warm off-white background #FAF9F8, warm taupe #B5A99A for borders.",
  "NO photograph, NO 3D rendering, NO logos, NO text labels embedded in the",
  "image (captions are added separately). Flat editorial vector style —",
  "The Atlantic / Kinfolk / a thoughtful financial-planning book. Aspect 1024x768.",
].join(" ");
...
function buildPrompt(title, surroundingProse) {
  const ctx = (surroundingProse || "")
    ...
    .trim()
    .slice(0, 800);
  return `${CANONICAL_PROMPT_PREFIX}\nThe diagram should illustrate: ${title}.\nContext: ${ctx}`;
}
